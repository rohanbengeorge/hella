...
//var no= local_Storage.nof_rows;
var nof_rows=parseInt(localStorage.nof_rows);
var temp_nof=0;
...
function Student(formElm) 
{
	...
	{
		temp_stu[i].name=arr.name;
		temp_stu[i].email=arr.email;
		temp_stu[i].dob=arr.dob;
		temp_stu[i].age=arr.age;
		temp_stu[i].address=arr.address;
...
function local_Storage()
{
	...
	this.getDetails= function(temp_arr)
	{
		...
		for (var i = 0; i < nof_rows; i++)
		{
			temp_arr[i].store_item(temp[i],temp_arr,i);
		}
		return temp_arr;
	...
	this.putDetails= function()
	{
		...
		{
			var obj= new Object();
			obj.name = temp[i].return_values(1);	
		    obj.email = temp[i].return_values(2);
		    obj.dob = temp[i].return_values(3);
		    obj.age = temp[i].return_values(4);
		    obj.address = temp[i].return_values(5);
...
function Table()
{
	...
	{
		//multiArray=obj_locStorage.intial(multiArray);
